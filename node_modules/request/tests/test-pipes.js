...
s.listen(s.port, function () {
  ...
  s.on('/cat', function (req, resp) {
    if (req.method === "GET") {
      resp.writeHead(200, {'content-type':'text/plain-tests', 'content-length':4});
      resp.end('asdf')
    } else if (req.method === "PUT") {
      assert.equal(req.headers['content-type'], 'text/plain-tests');
  ...
  request.get('http://localhost:3453/catresp', function (e, resp, body) {
    assert.equal(resp.headers['content-type'], 'text/plain-tests');
    assert.equal(resp.headers['content-length'], 4)
    check();
  })

  var doodleWrite = fs.createWriteStream(path.join(__dirname, 'tests.png'))

  counter++
  request.get('http://localhost:3453/doodle').pipe(doodleWrite)

  doodleWrite.on('close', function () {
    assert.deepEqual(fs.readFileSync(path.join(__dirname, 'googledoodle.png')), fs.readFileSync(path.join(__dirname, 'tests.png')))
    check()
  })

  process.on('exit', function () {
    fs.unlinkSync(path.join(__dirname, 'tests.png'))
  })
